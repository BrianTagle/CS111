...
void print_time_and_temp(int socketfd, double temperature)
{
  struct timespec ts;
  struct tm * tm;
...
void shutdown_func(void* v_sensors, int socketfd)
{
...
void input_handler(const char* input, mraa_sensors_t* sensors, int socketfd)
{
  ...
  ////////////////OFF//////////////////////
    ...
    {
      shutdown_func((void *) sensors, socketfd);
    }
  else
    {
      dprintf(socketfd, "Unrecognized command: \"%s\"\n", input);
    }
...
void main_handler(mraa_sensors_t* sensors, int socketfd)
{
  ...
  while(1)
      ...
      if(!stopReports)
	{
	  ...
	  print_time_and_temp(socketfd, temperature);
      ...
      while(time_diff(&begin, &end) < period)
	{
	  ...
	  if (ret > 0)
	      ...
	      for(i = 0; i < ret && current_char < (int)(sizeof(command_buf)); i++)
		{
		  if(buf[i] =='\n')
		    {
		      input_handler((char*)&command_buf, sensors, socketfd);
		      current_char = 0;
